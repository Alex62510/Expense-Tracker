...
const BarChart = ({ records }: { records: Record[] }) => {
  ...
  // Get color based on amount (используем оригинальные $)
  ...
  // Prepare chart data, конвертируем если ru
  const data = {
    ...
    datasets: [
      {
        data: aggregatedData.map((item) =>
          isRu ? item.amount * rate : item.amount,
        ),
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (context) {
            ...
            const total = isRu
              ? (item.amount * rate).toFixed(2)
              : item.amount.toFixed(2);
            const categoriesText =
              item.categories.length > 1
                ? `${t('chart.categories')}: ${item.categories.join(', ')}`
                : `${t('chart.category')}: ${item.categories[0]}`;
            return [
              `${t('chart.total')}: ${currencySymbol} ${total}`,
              categoriesText,
            ];
          },
        },
